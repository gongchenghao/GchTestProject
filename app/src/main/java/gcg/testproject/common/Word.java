package gcg.testproject.common;


import java.util.HashMap;
import java.util.Map;

/**
 * Created by Administrator on 2017/3/1 0001.
 */

public class Word {

    public static final int REQUEST_CODE = 0;

    public static final int REQUEST_CODE1 = 1; //选择图片

    public static final int REQUEST_CODE2 = 2; //删除图片

    public static final int REQUEST_CODE3 = 3; //选择视频

    public static final int REQUEST_CODE4 = 4; //拍摄视频

    public static final int REQUEST_CODE5 = 5; //删除视频

    public static final String FIRST_LAUNCH = "FIRST_LAUNCH"; //第一次加载

    public static final String TAOCAN_DETAIL = "TAOCAN_DETAIL";

    public static final String SERVICE_DETAIL ="SERVICE_DETAIL";

    public static final String RMB = "¥";

    public static final String NET_WEAK="网络不给力!";

    public static final String SERVICE_KIND ="fuwufenlei";

    public static final String REMIND_LOGIN = "请先登录！";

    public static final String  ASK_FAILED = "网络不给力，请重试！";

    public static float FIRST_SCROLLX = 0;//星系移动到中心点时的getScrollX()的值，在xingxifragment中赋值

    public static float FIRST_SCROLLY = 0;//星系移动到中心点时的getScrollY()的值，在xingxifragment中赋值

    public static int XING_XI_DP = 3800;//xml中设置的星系的dp值

    public static int XING_XI_PX = 5800;//给的星系图的px大小

}
